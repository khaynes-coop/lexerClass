...
public class GeneratorTests {
    ...
    private static Stream<Arguments> testSource() {
        Ast.Expression.Literal lit = new Ast.Expression.Literal(BigInteger.ONE);
        ...
        return Stream.of(
                Arguments.of("Hello, World!",
                        ...
                        new Ast.Source(
                                Arrays.asList(//init(new Ast.Global("name", "Integer" , false, Optional.of(lit)),
                                        //ast -> ast.setVariable(new Environment.Variable("name", "name", Environment.Type.INTEGER, true, Environment.create(BigInteger.ONE)))) ),
                                ),
    ...
    private static Stream<Arguments> testAccessExpression() {
        Ast.Expression.Access expr = new Ast.Expression.Access(Optional.empty(), "expr");
        expr.setVariable(new Environment.Variable("expr", "expr", Environment.Type.ANY, true, Environment.create("expr")));
        return Stream.of(
                Arguments.of("List",
                        // TRUE AND FALSE
                        init(new Ast.Expression.Access(Optional.of(expr), "list"), ast -> ast.setVariable(new Environment.Variable("list", "list", Environment.Type.ANY, true, Environment.create("list")))),
                        "list[expr]"
                )
        );
    }
