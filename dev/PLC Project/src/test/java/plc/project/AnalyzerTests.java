...
public final class AnalyzerTests {
    ...
    @MethodSource
    public void testGroup(String test, Ast.Expression.Group ast, Ast.Expression.Group expected) {
        ...
        return Stream.of(
                ...
                Arguments.of("Invalid Return",
                        new Ast.Expression.Group(
                            new Ast.Expression.Binary("+", new Ast.Expression.Literal(BigInteger.ONE), new Ast.Expression.Literal(BigInteger.TEN))
                        ),
                        init(
                        ...
                        ), ast -> ast.setType(Environment.Type.INTEGER)) ), ast -> ast.setType(Environment.Type.INTEGER))
    ...
    * */
    ...
    private static Stream<Arguments> testLiteralExpression() {
        return Stream.of(
                ...
                Arguments.of("Decimal Invalid",
                        // 9223372036854775807
                        new Ast.Expression.Literal(BigDecimal.valueOf(Double.MAX_VALUE).add(BigDecimal.valueOf(5.5))),
                        null
                )
        );
    }
    ...
    public void testFunctionExpression(String test, Ast.Expression.Function ast, Ast.Expression.Function expected) {
        test(ast, expected, init(new Scope(null), scope -> {
            scope.defineFunction("function", "function", Arrays.asList(Environment.Type.INTEGER), Environment.Type.INTEGER, args -> Environment.create(BigInteger.ONE));
        }));
    }
        ...
        return Stream.of(
                Arguments.of("Function",
                        ...
                        init(new Ast.Expression.Function("function", Arrays.asList(
                                init (new Ast.Expression.Literal(BigInteger.ONE), ast -> ast.setType(Environment.Type.INTEGER)))), ast -> ast.setFunction(new Environment.Function("function", "function", Arrays.asList(Environment.Type.INTEGER), Environment.Type.INTEGER, args -> Environment.create(BigInteger.ONE))))
                )
        );
    }
